...
import type { ExecResultFrame, GroupPageEntry, IdentityClaims, ToolDefinition } from '@zen-agent/contracts';
...
describe('toolgate adr-023 D3 — 定向副作用（目标页解析/围栏/通道分级/签名覆盖 targetPage）', () => {
  ...
  const groupPages: GroupPageEntry[] = [
    ...
    { handle: 'p2', url: 'https://seller.example/console/token', status: 'background' },
    { handle: 'p4', url: 'https://seller.example/console/inbox', status: 'silent' },
    { handle: 'p7', url: 'https://seller.example/account/settings', status: 'background' },
    { handle: 'p9', url: 'https://other.example/console/x', status: 'background' },
    { handle: 'workspace-view-00c3', url: 'https://seller.example/console/alt', status: 'background' },
  ];
  const refBatch = {
    task: '同站后台页提交',
    steps: [{ action: 'click', ref: 'za-2' }],
  ...
  it('generic pack 定向：同绑定 origin 的组内页可签，他源页 deny（围栏基准=激活时绑定的活跃页 origin）', async () => {
    const genericDomTool: ToolDefinition = {
      ...domTool,
      id: 'browse.page-operate',
      adapter: { kind: 'dom', pathPrefixes: ['/'] },
    };
    const port = createToolGatePort({ tools: [genericDomTool], signingSecret: SIGN_FIXTURE });
    const genericPages: GroupPageEntry[] = [
      { handle: 'p1', url: 'https://blog.example/posts/1', status: 'active' },
      { handle: 'p2', url: 'https://blog.example/posts/2', status: 'background' },
      { handle: 'p3', url: 'https://other.example/posts/9', status: 'background' },
    ];
    const base = {
      sessionId: 's-generic',
      toolCallId: 'c-generic',
      toolId: genericDomTool.id,
      claims: validClaims,
      packOrigin: 'https://blog.example',
      domContext: {
        refs: ['za-2'],
        path: '/posts/2',
        origin: 'https://blog.example',
        url: 'https://blog.example/posts/2',
        pageInstanceId: 'page-instance-2',
      },
