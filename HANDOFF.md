...
- **真实 LLM 已接通并端到端验证**（`pnpm test:e2e:real`，见 `evals/runs/2026-07-04-real-llm.md`）：13 场景行为断言全绿 + Workflow 并行质量/治理判定 13pass/1concern（forbidden 纵深防御两层齐备）。修复了两处 mock 掩盖的真实问题：① 回喂缺 assistant `tool_calls` 回声（加法式补 `LlmMessage.toolCalls`）；② `ZA-FEAT-03`/`ZA-SYS-03` 原让 LLM 自行文本确认、绕过服务端 HITL 治理（已改为确认交平台）。
...
| 事项 | 现状 | 触发锚点 |
|---|---|---|
| ~~LLM 仅确定性 mock~~ 真实 provider 已接通 | `pnpm test:e2e:real` 端到端跑真实 LLM 13 场景（行为绿 + Workflow 质量判定）；mock 仍保留供确定性回归 | 已了结（2026-07-04），见 `evals/runs/2026-07-04-real-llm.md` |
| ~~`LlmMessage` 无 assistant `tool_calls` 回声~~ 已补 | 加法式加 `LlmMessage.toolCalls` + `LlmToolCall`，gateway 回声、llm-port 映射 OpenAI `tool_calls` | 已了结（2026-07-04） |
| forbidden 真实 LLM 常文本自拒、不触发 toolgate deny 事件 | 结果安全；服务端 deny 由 mock 评测(同 server 二进制)+toolgate 单测覆盖（纵深防御第二层） | 如需真实 LLM 侧也演示第二层，加对抗用例强制工具调用（当前非必需） |
