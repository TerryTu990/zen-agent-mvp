...
>
> **奠基后演进勘误**（本文件保留历史原貌，以下事实已被后续 ADR 取代，冲突处以勘误为准）：
> - `server` 执行通道已在 MVP 实现（adr-010 服务端直调 + credentialRef 凭证注入），不再是"定形不实现"。
> - 新增 **dom 执行通道**（adr-011 可见页面代操作：page_snapshot 观察 + dom 批次步进 + 任务级授权）。
> - 会话模型改为**显式会话组**（adr-012 会话=标签组，点图标建组）+ **跨站任务组**（adr-013）。
...
**奠基后增补**（详见各 ADR 原文，此处只留指针）：

- **D10（adr-010）API 调用形式**：server 通道在 MVP 落地——服务端直调 + `credentialRef` 运行时凭证注入，与 client 代执行并存，通道仍是工具定义的配置维度。
- **D11（adr-011）可见页面代操作**：取代 D9 的"不做"——dom 通道以"用户全程可见 + 每步高亮 + 服务端签发闭集步骤 + 任务级授权"的形态落地；`page_snapshot` 观察半程 + dom 批次步进器。
- **D12（adr-012）会话=标签组**：显式点图标建组，拖 tab 入组即入会话，关组即关会话（被 adr-013 会话组语义吸收）。
- **D13（adr-013）站点包与跨站任务组**：registry/pack 两级装配、site 围栏、per-origin 身份、site_navigate 跨站导航、上下文治理 P0-P2、任务级 HITL 授权。
