...
> 人读层参考文档。事实权威：奠基期以 `00-design-brief.md`（SSOT）为准，契约细节见 `02-contracts.md` 与各 `.schema.json`；本文负责解释结构、边界、流程与权衡。
> 决策"为什么"见 `../adr/`（D1-D9），分期计划见 `../roadmap.md`。
...
│  │ (⑤ 消费端)  │ │ 指令签发/回收│ │ 白名单插拔    │ │ 事件旁路      │  │
│  └───┬────────┘ └─────────────┘ └──────────────┘ └─┬────────────┘  │
└──────┼─────────────────────────────────────────────┼───────────────┘
       │ 读取版本化快照（C4）                            │ 脱敏落盘（C5）
┌──────▼─────────────────────────┐        ┌──────────▼──────────────┐
│ ⑤ assets/（git 文件即配置）      │        │ .za/events.jsonl        │
│  features/<id>/{feature.md,    │        │ （会话/装配/工具决策/     │
│  facts.md, tools.json}         │        │   执行/HITL 全链路事件）  │
│  skills/<fn>/SKILL.md          │        └─────────────────────────┘
│  manifest.json                 │
└────────────────────────────────┘
```

会话状态存本地（SQLite/文件），但**接口按可外置设计**——这是 S4 状态外置的前提。
...
- 约束：C1 工具定义的 `execution` MUST 取 `'client' | 'server'` 闭集；MVP 只实现 client 通道，`server` 枚举值 MUST 保留不删，toolgate 遇到未实现通道 MUST fail-closed 拒绝而非降级。
- 如何保平滑：S1 上线服务端直调时，只需在 ③ 补 server 执行器 + 在工具定义里改一个字段——agent、网关、客户端全部无感（adr-003：通道是配置维度）。
- 违反代价：若 MVP 把"客户端执行"写死进类型或流程分支，S1 就是一次全链路改造。
...
- 如何保平滑：安全模型从 MVP 第一天就是标准版形态——S1 加 server 通道、S4 拆 ③ 为独立服务时，信任边界不移动、不需要重新安全评审整个链路。
...
| DOM 自动化需求被客户提出而 MVP 不做 | 预期落差 | adr-009 已定锚点：标准版后按真实需求评估；引导高亮+API 代执行覆盖主要价值 |
