...
**有界自动履约（adr-016）**：确定性履约工具可声明业务键映射；toolgate 只在服务端预批准策略同时命中已验签账号、工具、商品、有效期、单笔数量和每日限额时自动放行，并原子预占订单。未命中或超界回到 HITL；失败/超时/结果不明记 `uncertain`，同订单不自动重试。策略不进客户端、pack 或模型上下文。
