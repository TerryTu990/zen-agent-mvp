...
**有界自动履约（adr-016）**：可信连接器先通过进程内端口登记一次性履约意图，绑定账号、精确页面 URL、商品、规范化订单、数量与固定 DOM 步骤；模型工具只传 opaque `intentId`。toolgate 同时匹配服务端策略并原子预占全局订单键，签名指令直接取意图步骤。缺失/错页/超界即 deny 并转独立人工工具；失败、主动 TTL 超时或结果不明记 `uncertain`，同订单不自动重试。策略与意图不进客户端、pack 业务内容或模型上下文。
