...
- **账号登录（正式投产前置条件）**：当前身份形态是匿名自动登录（安装 id → 短期 JWT，adr-022），无持有证明、与设备绑定。面向真实用户投产（托管服务开放 / 商店正式发行）前 **MUST** 完成 Google 账号登录（OIDC，绑定链路见 adr-014 §1）并做一次性 subject 迁移；访问控制与用量配额同期落地。
...
| 项 | 来源 | 触发锚点 | 状态 |
|---|---|---|---|
| gateway/extension 去 xianyu 硬编码 | 本次合并（方案 A） | P1；最迟第二个真实站点 pack 接入时 | **已了结**（2026-08-04，adr-019 批次①-④；验证记录 `../../evals/runs/2026-08-04-adr019-batch2.md`。残留：sidepanel/manifest 品牌文案归 P2；background 旧存储键字面量属迁移函数必要数据） |
...
| Google 账号登录（OIDC） | adr-022 / adr-014 §1 | **正式投产前置条件**：托管服务开放 / 商店正式发行前 MUST 完成；实施期 P4 | 未了结 |
| 访问控制与用量配额（按 subject 限量） | adr-022 | P4 托管服务启动时，或匿名激活出现滥用信号时 | 未了结 |
| 跨设备同步与 subject 迁移（含「重置身份」入口） | adr-022 / adr-014 §1 | Google 登录上线时（在此之前重置会孤立该 subject 的 L2 overlay） | 未了结 |
