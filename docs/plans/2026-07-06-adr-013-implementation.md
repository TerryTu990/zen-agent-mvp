...
| 观察 | 影响 | 锚点 |
|---|---|---|
| 多次压缩下摘要内嵌的边界标记/task 计划会被再摘要糊掉（`compress.ts`） | 长会话保真度退化；授权语义不受影响（hitlGrants 不进历史）；验收路径（40 轮上限）实际不可达二次压缩 | 真实宿主出现超长会话（单会话 >2 次压缩）时改为结构化透传 |
| 摘要/边界标记前缀启发式可被同形用户输入伪冒 | 最坏仅回合切分点偏移，无安全后果 | 同上，随结构化透传一并处理 |
| llm-port 每条 SSE data 行 JSON.parse 两次 | 纯效率，量级微小 | llm-port 下次实质性改动时顺带合并 |
