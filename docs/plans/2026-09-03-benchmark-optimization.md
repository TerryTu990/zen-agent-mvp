...
## 4. 待 Terry 裁决（8 项）——**已于 2026-09-03 全部裁决**

裁决原文与下一轮工作序列见 `2026-09-03-terry-rulings-and-next-round.md`；
交付报告 §6 有摘要表。本节原文保留在 git 历史中，此处不再复述以免两处口径漂移。

**对 §3「本轮不做」表的影响**：三条锚点提前到下一轮——用户级站点黑名单（因 `ZA_GENERIC_ALLOWLIST` 被裁掉，
不做即真空）、不可信内容结构化定界（与 R8 改写合并共用一次全量评测）、权限最小化注入（同一隐私面）。
表中「快捷指令库、BYOK 多模型、会话历史」一行的「待 Terry 裁决」已解除：快捷指令下一轮做，
会话历史进 P3.5，BYOK 留 P4。
