...
| E2E m5 | `pnpm test:e2e:m5` | **未跑（不在任何门清单里）** | **基线即红 → 绿（2026-09-05 溯源，`b5304c9`）**：自 `3117f29`（2026-07-23）起红——服务端停止改为就地收尾、不再回喂模型总结，同 commit 更新了单测与 sidepanel 判据却漏了 m5，两个多月无人发现。见 §5 |
| 面板 E2E | `pnpm test:e2e:sidepanel` | 记为红（401 阶段） | **绿（2026-09-05 溯源后修 harness，`f0ea8ba`，10/10）**：此前的「不稳定」是夹具竞态——`b691932` 把用户气泡渲染前移到提交瞬间，夹具仍拿它当「上行帧已到达」的屏障。见 §5 |
...
| 事项 | 状态 | 锚点 |
|---|---|---|
| `pnpm test:e2e:sidepanel` **不稳定 → 已闭合（2026-09-05，`f0ea8ba`）** | **根因已查明**：夹具把「面板出现用户气泡 + 输入框清空」当作上行帧已到达的 happens-before 屏障，而 `b691932`（2026-08-09）有意把回显前移到提交瞬间（父 `ad8c365` 10/10 绿 → 该点 9/10 红），401/404 重试路径多一次建会话往返、窗口最宽，按下标读 `frameRequests[n]` 拿到 undefined 即红；`:288` 的 409 幂等判据在帧缺失时反而假绿。修法：三处断言前先等帧计数到达，取值比较一字未动；负控（多签一枚令牌）仍红。**此前的「两次更正」经过保留如下作记录**。①原始记录称基线与终态皆红（401→404 阶段）；②2026-09-03 复核在 HEAD 连跑 14 次全绿（含 3 次满负载），据此判原记录为误判并更正了三处文档；③2026-09-04 第三方验证在**同一 commit `1b17dd0` 的独立 git worktree** 里重跑，3/3 红，失败点正是「401 后重试未使用重新激活的令牌」——与①的描述一致。故②的更正**下重了**：真相既不是稳定红也不是稳定绿，而是**跨环境/跨时间不确定**。原始记录对它当时的环境很可能是准确的。根因未查明（疑为 401 令牌续期路径的竞态）| 单独立项：作为不稳定门专项诊断，不计入批次成败 |
...
| 侧边栏无就地「在本站启用」入口（N5，产品残余面） | 未授权 origin 上轨一只在手势那一份文档内有效；跨 origin 导航后 `executeScript` 缺 host 权限即失败，该页无 content（不降级、不改投，服务端按目标不可达处置），用户须去配置中心授权 | P3 商店上架权限模型评审时补面板侧入口 |
| activeTab 手势授权路径本身不在 E2E 覆盖内（N5，harness 残余面） | 工具栏与授权气泡都是浏览器 UI，Playwright 点不到（`chrome.permissions.request` headed 下挂起、headless 下扩展 SW 起不来）；夹具以「已授权站点访问」的清单装载替代，被测的仍是注入时机 | Playwright 支持扩展工具栏/权限气泡自动化时 |
| 配置中心的本机权限对账只发生在打开页面那一刻（N5 修复，minor） | 页面已开着时用户在 chrome://extensions 撤销站点访问，本页不即时感知，下次打开即如实；未加 `permissions.onRemoved` 重探是刻意的简洁取舍 | 用户反馈需要时加事件重探 |
| `storage.onChanged` 两键同批场景下 `revokeStaleActivationSkips` 未被吞掉这一半无断言（N5 minor 收口复核，minor） | 新用例只钉了注册面对齐与组页面清单重报；denylist-only 既有用例不覆盖同批形态 | 下次动 `background.ts` 配置变更分发时补一条 |
| 站点包自动化行的 `pack.origin` 过不了授权文法（带路径/通配）时静默不给授权入口（N5 minor 收口复核，minor） | `appendGrantEntry` 对 origin 为 null 的四种成因之一未提示；是否可达取决于 pack.origin 契约约束，未展开核 | 收紧 pack.json `origin` 文法或补提示时 |
| 历史裁决/计划记录里的 `<all_urls>` 与 `za.autoActivate` 表述保留原文（N5，登记） | 裁决记录 N2 论证「内容脚本是 `<all_urls>` 全站注入，删除它不新增暴露面」与 l0-l3 编排计划的 autoActivate 说明是当时记录，改写等于篡改；现行口径以 adr-027 为准 | — |
| `pnpm test:e2e:m5` 自 `3117f29` 起静默红了两个多月（溯源，已闭合 `b5304c9`） | 服务端停止改为「就地收尾、零模型轮」（adr-024 D2）后，m5 仍等 mock 的「已按用户要求停止」总结；该 commit 更新了 server 单测与 sidepanel 判据却漏了 m5，且 m5 不在任何门清单里。新判据三条可证伪（余下步骤零执行 / user-stopped 后紧接服务端收尾 / 零模型轮），已用变异实跑证明会转红 | 已闭合；`gates.sh` 与门表已纳入 m5 |
| 执行中点停止后面板出现无意义的「HTTP 409」（溯源探针实证，产品缺陷） | `handleStop` 合成 user-stopped 回执并摘除 nonce，content 侧步间检查点迟到的真实回执按一次性语义被拒 409 并渲染成错误 | 已修（`4acd397`）：合成时记 nonce，迟到同 nonce 回执幂等受理；未知 nonce 与二次迟到仍 409 |
| `apps/extension/src/identity.ts` 两处疑似竞态（溯源读码，未观察到触发，minor） | ① `refreshToken()` 只清缓存即调 `activate()`，若 401 到来时恰有更早的激活在途会复用其结果；② 清缓存到写回 storage 之间并发 `getToken()` 可能把未过期的旧令牌回填 | 401 恢复链路再出现间歇红时优先核这两处 |
...
1. **门可能在基线就是红的，而此前没人知道**：`pnpm test:e2e` 的断言与 2026-09-03 改版后的基座相反，本轮修好；
   ...
   **后记（2026-09-05）**：立项溯源后两门都是 harness 没跟上有意的产品语义变更——sidepanel 是提交即回显打破了夹具的 happens-before 假设，
   m5 是停止改为就地收尾后判据仍等模型总结；前者间歇、后者恒红两个多月，共同点是**改产品语义的那个 commit 只更新了它想到的判据**。
