...
1. **R8 拒答边界的规范性改写**（r1 A-UX-01/A-DOC-01/A-ASM-04）：2026-09-03 基座已通用化、拒答维度已退出评测，
   但 `product-form-definition.md` §3 的 R8 与 SSOT §1 仍以 MUST 要求拒答。
   建议改写方向与影响面已写入 **adr-025**（本轮补记该定位级变更，此前无任何 ADR）。
2. **注入透明视图的产品定位**（r1 A-UX-02）：抽屉已在 `11454c4` 主动删除；本轮只做了面板内的最小「本页生效」块。
   R4 互证条款与北极星验收措辞待裁决。
3. **L0 运营者配置的 U4 豁免准则**（r1 A-ARCH-08）：`ZA_GENERIC_ALLOWLIST` / `ZA_FULFILLMENT_*` 是快照与 L2 之外的第三类配置源。
4. **履约语义进核心契约的取舍**（r1 A-ARCH-04）：`CardInventoryPort` / `FulfillmentCoordinatorPort` 把电商发货/送卡语义固化进 C6。
5. **快捷指令库**（模式卡 PC-CGB2-03 / PC-CGB3-01 / PC-PROD-03）是否进产品形态。
6. **BYOK 多 provider 与会话历史列表**（PC-CGB4-01/02/03、PC-PROD-07/08）的分期归属（P3.5 还是 P4）。
7. **ZA-SYS-04 与 ZA-SYS-01 在 generic 页面的张力**（r1 A-ASM-06）：基座既要求「站点功能未覆盖即明确不确认」，又定位为通用助手。
8. **本轮未做项的锚点安排**是否接受（方案 §3 表）。
