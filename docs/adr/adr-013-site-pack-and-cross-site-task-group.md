...
提议（2026-07-05 初稿；同日 v2 按评审意见修订 location 匹配/迁移/实改面/参照；同日 v3 并入会话上下文管理 P0-P2，待 Terry 复审）
...
- 分期：**① P0 观测瘦身 + P2 会话持久化**（小改动、独立收益，先行）；**② pack 机制**（demo config 迁为首个 pack；codeflow pack 的 facts 纳入站点组件库交互提示——E2E 实证：一条"Semi 下拉须点选项行"可省数轮摸索）；**③ P1 历史压缩**；**④ 任务组（§5）**——锚点：第二个真实站点 pack 接入且出现跨站业务场景，且 P1 已就位。
