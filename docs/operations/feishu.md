...
服务器目录 `/root/zen-agent/lark-cli` 只由管理员和服务容器使用；compose 将其挂载为 `/data/lark-cli`，并设置 `LARKSUITE_CLI_CONFIG_DIR=/data/lark-cli`。目录必须允许容器 `node` 用户读取并在 token 刷新时写回，容器重建不得清空。
