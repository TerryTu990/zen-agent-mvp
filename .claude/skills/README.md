...
| `zen-eval-harness` | 原锚点（评测集建立）已触发，runner 已自建于 `scripts/evals/run.mjs`；重挂锚点：判据形态大改时重估是否还要移植 | dataset 种子源改为 `assets/packs/<packId>/` + `examples/host-demo`；维度对齐 `ZA-C-EVAL-01` 闭集（讲解/装配换出/引导/工具/HITL） |
