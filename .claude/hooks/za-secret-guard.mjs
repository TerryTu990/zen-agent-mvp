 ...
 *
 * PreToolUse hook，作用于会落盘/执行的写工具（Bash|Edit|Write|MultiEdit）与会取值的读工具（Read|Grep）。
 * 写/执行面从 tool_input 取待写内容（Edit/Write 的 content/new_string、MultiEdit 遍历 edits、
 * Bash 的 command）扫 SEC-01/02/03；读面从 Read.file_path / Grep.path 判 SEC-03 凭证闭集；
 * 命中即把"红线编号 + 修法"写 stderr 并 exit 2
 * （Claude Code 契约：PreToolUse exit 2 拦截工具并将 stderr 反馈给模型）。
 *
 * 设计不变量：
...
const GUARDED_TOOLS = new Set(['Bash', 'Edit', 'Write', 'MultiEdit', 'Read', 'Grep']);
...
/** 取本次工具的待写文本：Edit/Write→content/new_string，MultiEdit→所有 new_string；Bash 单独走命令检测。 */
