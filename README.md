...
| `pnpm test:e2e`<br>（`:m2` `:m3` `:m5` `:sidepanel` `:coldstart` `:d3` `:xianyu`） | 分场景端到端脚本（`scripts/e2e/run-*.mjs`），各自拉起服务端并驱动扩展 |
...
| `pnpm verify:phase2` / `pnpm verify:phase3` | 更大范围验证门：契约/快照/履约用例定点回归（phase2）、xianyu + sidepanel e2e（phase3） |
