#!/usr/bin/env node
/**
 * 开发期 Codex Stop 验证门 hook。
 *
 * 在 turn 收尾（声称完成的时刻）把 ZA-C-WHEN-02「验证通过才称完成」推到模型面前：
 * 若工作树存在 git-dirty 的 *.ts/*.mts 改动，续发一轮提示，建议先跑构建+测试再汇报完成。
 *
 * 设计不变量：
 * - 有未验证 TS 改动时用 Codex `decision:block` 续发一轮验证提示；不是工具执行拦截。
 * - 防自循环：本 hook 的 continuation 会唤醒主对话，其收尾再次触发 Stop 时输入带
