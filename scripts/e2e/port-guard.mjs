 ...
 * 先占位探测把这种串台变成开跑前的硬失败。
 * 失败以抛错表达而非直接退出进程：各脚本既有的 catch/finally 仍能落 status:failed 的证据并按原路径退出非零，
 * 磁盘上的 result.json 与家族报告的 exitCode 才一致。
 */
...
export class PortsBusyError extends Error {
  constructor(busy) {
    super(
      `端口被占用，拒绝开跑（可能另一 worktree 正占用；改用 ZA_E2E_*/ZA_EVAL_* 端口 env 隔离）：\n  ${busy.join('\n  ')}`,
    );
    this.name = 'PortsBusyError';
    this.busy = busy;
  }
}
 ...
 * 任一端口不可绑定即抛 PortsBusyError，消息含占用清单；全部空闲则正常返回。
 */
export async function assertPortsFree(entries) {
  ...
  if (busy.length > 0) throw new PortsBusyError(busy);
}
